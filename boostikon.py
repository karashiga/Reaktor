...
def startup_animation():
    ...
    # Animation frames
    frames = [
        "🚀 Initializing DIGILAB...",
        "⚡ Loading modules...",
        "🔧 Setting up environment...",
        "📁 Creating directories...",
        "🎨 Loading UI components...",
        "🔥 Preparing Facebook API...",
    ...
    for i, frame in enumerate(frames):
        print(f"\r    {yellow}{frame}{'.' * (3 - i % 3)}", end='', flush=True)
        time.sleep(0.6)
    
    print(f"\r    {green}✅ DIGILAB System Ready!{' ' * 20}\n")
    time.sleep(1)

def loading_animation(text):
    """Show loading animation for operations"""
    print(f"\r    {yellow}{text}", end='', flush=True)
    for i in range(3):
        time.sleep(0.3)
        print(".", end='', flush=True)
    print(f"    {green}✅ Done!\n")

def progress_animation(current, total, text):
    """Show progress animation"""
    percentage = (current / total) * 100
    bars = int(percentage / 5)
    progress = "█" * bars + "░" * (20 - bars)
    print(f"\r    {text} [{progress}] {percentage:.1f}%", end='', flush=True)

def success_animation():
    """Show success animation"""
    print(f"\r    {green}🎉 SUCCESS!{' ' * 30}\n")
    time.sleep(0.5)

def error_animation():
    """Show error animation"""
    print(f"\r    {red}❌ ERROR!{' ' * 30}\n")
    time.sleep(0.5)

def ensure_directories_and_files():
    """Create jrmboost directory and all required files"""
    jrmboost_dir = "/sdcard/jrmboost"
    
    try:
        # Create directory if it doesn't exist
        os.makedirs(jrmboost_dir, exist_ok=True)
        
        # Create all required files
        required_files = [
            f"{jrmboost_dir}/FRAACCOUNT.txt",
            f"{jrmboost_dir}/FRAPAGES.txt", 
            f"{jrmboost_dir}/RPWACCOUNT.txt",
            f"{jrmboost_dir}/RPWPAGES.txt"
        ]
        ...
        for file_path in required_files:
            ...
            if not os.path.exists(file_path):
                ...
                print(f"    {green}📁 Created: {file_path}")
        
        print(f"    {green}✅ All directories and files ready!")
        return True
        
    except Exception as e:
        print(f"    {red}❌ Error creating directories: {e}")
        return False
...
def jovan():
    ...
    {adrkz} 
         ...
         ╚═╝  ╚═╝ ╚═════╝ ╚══════╝ ╚═════╝ ╚═╝     ╚═╝╚══════╝
     {blue}───────────────────────────────────────────────────────────────\033[0m
     {blue}OWNER     {yellow}: {green}Jhames Martin
     {blue}VERSION   {yellow}: {green}Jrmph {red}V1 {yellow}/ {adrkz}CELLPHONE VERSION
     {blue}FACEBOOK  {yellow}: {green}https://www.facebook.com/profile.php?id=61569198136653
    ...
    if os.path.exists(token_output_path):
        try:
            with open(token_output_path, 'r') as f:
                existing_uids = {line.split('|')[0] for line in f.readlines() if line.strip()}
        except Exception as e:
            print(f"    {yellow}⚠️ Warning: Could not read existing tokens file: {e}")
...
def axl1():
    ...
    print(f"    \033[34m───────────────────────────────────────────────────────────────\033[0m")
    print(f"     \033[33m✅ USING DIRECT PATH: {account_file}")
    print(f"     \033[33m📝 FORMAT: uid|pass")
    print(f"    \033[34m───────────────────────────────────────────────────────────────\033[0m")
    print(f"     \033[33m💡 TIP: Add your accounts in {os.path.basename(account_file)} file")
    print(f"     \033[33m   Example: 1234567890|password123")
    print(f"    \033[34m───────────────────────────────────────────────────────────────\033[0m")
    
    # Check if file exists and has content
    if not os.path.exists(account_file):
        print(f"    {yellow}⚠️ Creating file: {account_file}")
        ...
        with open(account_file, 'r') as f:
            ...
            if lines:
                print(f"    {green}✅ Found {len(lines)} accounts in file")
            else:
                print(f"    {yellow}⚠️ File is empty. Add accounts first!")

    # Check if we should proceed (file has content)
    if not os.path.exists(account_file) or (os.path.exists(account_file) and len([line.strip() for line in open(account_file, 'r').readlines() if line.strip()]) == 0):
        print(f"    {yellow}⚠️ No accounts found in {os.path.basename(account_file)}")
        print(f"    {yellow}Please add accounts in the format: uid|password")
        print(f"    {yellow}Example: 1234567890|password123")
        print(f"    {yellow}Then run again to extract tokens!")
        input(f"     {green}Press Enter to continue...")
        return

    # Show extraction progress
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f"     {green}🎯 EXTRACTING TOKENS FROM {os.path.basename(account_file)}...")
    loading_animation("📥 Loading accounts...")
    
    token_output_path = account_file
    proz(account_file, token_output_path, extract_type)
...
def axl2():
    ...
    prozc(account_file, account_file, extract_type)
...
def remove_duplicates():
    ...
    jovan()
    file_paths = {
        "1": "/sdcard/jrmboost/FRAACCOUNT.txt",
        "2": "/sdcard/jrmboost/FRAPAGES.txt",
        "3": "/sdcard/jrmboost/RPWACCOUNT.txt",
        "4": "/sdcard/jrmboost/RPWPAGES.txt"
    }
...
def perform_reaction_fast_vid():
    ...
    post_id = Video_Extractid(post_link)  # Assuming Video_Extractid is defined elsewhere
    uid_url = post_id  
    ...
    reactions_count = 0
    max_workers = 5  # Set maximum number of threads
    results = []  
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_token):
                ...
                try:
                    ...
                    if status_code == 200:
                        reactions_count += 1
                        print(f"   \033[1;31m[SUCCESS]\033[1;31m \033[1;32mSUCCESSFULLY REACTED\033[1;32m: {reactions_count}/{num_reactions}")
                    else:
                        print(f"   \033[1;31m[FAILURE]\033[1;31m {num_reactions}/{reactions_count} ")
                except Exception as e:
...
def AutoReact():
    ...
    def get_ids_tokens(file_path):
        try:
            ...
            print(f"    {red}❌ Error reading file {file_path}: {e}")
            return []
...
def perform_reaction_reels():
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        while total_successful_reactions < num_reactions and token_index < len(tokens):
            future_to_token = {
                executor.submit(react_to_reels, tokens[token_index], reels_link, reaction_type): token_index
            }
            for future in as_completed(future_to_token):
                try:
                    access_token, status_code, response_text = future.result()
                    ...
                    print(f"Error processing token {token_index}: {e}")
            token_index += 1
...
def auto_follow_fast():
    ...
    jovan()  # Assuming you have a function to display your logo or intro
    ...
    print(f"    {green}FORMAT {yellow}: {red}https://www.facebook.com/profile.php?id=100078043222260")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    a = input(f'    {green}Enter the target account ID: ')
    account_id = extract_user_id_prof(a)  # Assuming you have this function
    if not account_id:
        print(f"Invalid account ID {a}")
        return
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    ...
    try:
        ...
        print(f"Invalid number for follow limit.")
        return
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")

    follow_count = 0
    failed_count = 0  # Keep track of failed follows
    total_tokens = len(tokens)  # Total number of tokens available
    current_index = 0  # Current index in the token list

    with ThreadPoolExecutor(max_workers=20) as executor:
        while follow_count < follow_limit and current_index < total_tokens:
            token = tokens[current_index]  # Get the current token
            # Assuming tokens are in email|token format
            page_access_token = token.split('|')[1]  # Extract the token from email|token format

            # Submit the follow task to the executor
            task = executor.submit(follow_account, page_access_token, account_id)
            success = task.result()  

            if success:
                follow_count += 1
                print(f"     \033[1;31m[SUCCESS]\033[1;32m SUCCESSFULLY FOLLOWED\033[0m")
            else:
                failed_count += 1
                print(f"     {red}[UNSUCCESSFUL]\033[1;31m UNSUCCESSFULLY FOLLOWED\033[0m")

            current_index += 1  

    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f'     {red}Total successfully followed: {follow_count}')
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f'     {red}Total failed follows: {failed_count}')
...
# Approval system removed - direct access to all features
# All original functionality preserved including page creator system
...
def report_account():
    ...
    print(f"""     {yellow}ACCOUNT REPORT GENERATOR:
     {blue}───────────────────────────────────────────────────────────────\033[0m""")
    
    file_options = {
        1: ("FRA ACCOUNT", "/sdcard/jrmboost/FRAACCOUNT.txt"),
        2: ("FRA PAGES", "/sdcard/jrmboost/FRAPAGES.txt"),
        3: ("RPW ACCOUNT", "/sdcard/jrmboost/RPWACCOUNT.txt"),
        4: ("RPW PAGES", "/sdcard/jrmboost/RPWPAGES.txt")
    }
    
    print(f"     {white}SELECT FILE TO REPORT:")
    for num, (name, path) in file_options.items():
        print(f"     {blue}[{num}] {green}{name}")
    ...
    name, file_path = file_options[choice]
    
    # Loading animation
    loading_animation("📊 Analyzing accounts...")
    ...
    try:
        ...
        if not lines:
            print(f"    {yellow}⚠️ {name} file is empty")
            input(f"     {green}Press Enter to continue...")
            return
        ...
        print(f"""     {yellow}📋 DETAILED {name.upper()} REPORT
     {blue}───────────────────────────────────────────────────────────────\033[0m""")
        
        print(f"     {white}📊 STATISTICS:")
        print(f"     • Total entries: {green}{len(lines)}")
        ...
        print(f"     • Valid format: {green}{valid_entries}")
        print(f"     • Invalid format: {red}{invalid_entries}")
        print(f"     • Email entries: {blue}{email_count}")
        print(f"     • UID entries: {blue}{uid_count}")
        
        print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
        
        # Show sample entries
        print(f"     {white}📝 SAMPLE ENTRIES (First 5):")
        for i, line in enumerate(lines[:5]):
            if '|' in line:
                parts = line.split('|')
                if len(parts) == 2:
                    print(f"     {i+1}. {parts[0][:20]}{'...' if len(parts[0]) > 20 else ''}|{parts[1][:20]}{'...' if len(parts[1]) > 20 else ''}")
                else:
                    print(f"     {i+1}. {red}INVALID FORMAT: {line[:30]}...")
            else:
                print(f"     {i+1}. {red}INVALID FORMAT: {line[:30]}...")
        ...
        print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
        print(f"     {yellow}💡 RECOMMENDATIONS:")
        if invalid_entries > 0:
            print(f"     • {red}Remove {invalid_entries} invalid entries")
        if email_count == 0 and uid_count == 0:
            print(f"     • {yellow}Add valid entries in uid|pass format")
        else:
            print(f"     • {green}File format looks good!")
...
def create_file():
    """Show file information and instructions"""
    clear_screen()
    jovan()
    
    print(f"""     {yellow}FILE MANAGEMENT GUIDE:
     {blue}───────────────────────────────────────────────────────────────\033[0m""")
    
    file_options = {
        1: ("FRAACCOUNT.txt", "Store Facebook account credentials (uid|pass)"),
        2: ("FRAPAGES.txt", "Store Facebook page tokens (uid|access_token)"), 
        3: ("RPWACCOUNT.txt", "Store RPW account credentials (uid|pass)"),
        4: ("RPWPAGES.txt", "Store RPW page tokens (uid|access_token)")
    }
    
    for num, (filename, description) in file_options.items():
        full_path = f"/sdcard/jrmboost/{filename}"
        print(f"     {blue}[{num}] {yellow}{filename}")
        print(f"          {white}{description}")
        print(f"          {green}📍 Location: {full_path}")
        ...
        if os.path.exists(full_path):
            with open(full_path, 'r') as f:
                lines = [line.strip() for line in f.readlines() if line.strip()]
                if lines:
                    print(f"          {green}✅ Status: {len(lines)} entries")
                else:
                    print(f"          {yellow}⚠️ Status: Empty file")
        else:
            print(f"          {red}❌ Status: Not found")
        print()
    
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f"     {white}📝 FORMAT EXAMPLES:")
    print(f"     {green}ACCOUNTS (uid|pass):")
    print(f"          1234567890|password123")
    print(f"          user@example.com|mypassword")
    print(f"     {green}TOKENS (uid|access_token):")
    print(f"          1234567890|EAAxxxxxxxxxxxx")
    print(f"          user@example.com|EAAxxxxxxxxxxxx")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f"     {yellow}💡 QUICK TIPS:")
    print(f"     • Use Choice 1 (START) to extract tokens from accounts")
    print(f"     • One account/token per line")
    print(f"     • Files are auto-created in /sdcard/jrmboost/")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    
    input(f"     {green}Press Enter to go back to main menu...")
...
def main():
    ...
    if not ensure_directories_and_files():
        print(f"    {red}❌ Failed to setup directories. Exiting...")
        return
    
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    
    # Initialize file paths
    fraaccounts_file = '/sdcard/jrmboost/FRAACCOUNT.txt'
    frapages_file = '/sdcard/jrmboost/FRAPAGES.txt'
    rpwaccounts = '/sdcard/jrmboost/RPWACCOUNT.txt'
    rpwpages = '/sdcard/jrmboost/RPWPAGES.txt'
    ...
    clear_screen()
    jovan()
    print(f"""                 {lightblue}JRMPH V1 - FILE STATUS OVERVIEW💫
          
                            {blue}FRA ACCOUNT{yellow} : {green}{total_accounts} {white}(/sdcard/jrmboost/FRAACCOUNT.txt)
                            {blue}FRA PAGES  {yellow} : {green}{total_pages} {white}(/sdcard/jrmboost/FRAPAGES.txt)
                            {blue}RPW ACCOUNT{yellow} : {green}{ total_account_rpw} {white}(/sdcard/jrmboost/RPWACCOUNT.txt)
                            {blue}RPW PAGES  {yellow} : {green}{total_pages_rpw} {white}(/sdcard/jrmboost/RPWPAGES.txt)
      {blue}───────────────────────────────────────────────────────────────\033[0m""")
    print(f"     {yellow}💡 All files auto-created in /sdcard/jrmboost/ directory")
    print(f"     {yellow}💡 No need to specify paths - files used directly!")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    ...
    print(f"      {yellow}[13]  {blue}REACT TO POST (W CARE)    {yellow}- {green}[PAGE & ACCOUNT]")
    print(f"      {yellow}[14]  {blue}AUTO GROUP JOIN           {yellow}- {green}[PAGE & ACCOUNT]")
    print(f"      {red}[15]  {red}RESET")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f"                         {red}ADDITIONALS 🔥                                 ")
    print()
    print(f"      {yellow}[16] {blue}AUTO CREATE PAGE          {yellow}- {green}[PHB NAMES]")
    print(f"      {yellow}[17] {blue}AUTO SET PFP              {yellow}- {green}[RANDOM]")
    print(f"      {yellow}[18] {blue}CREATE FILE               {yellow}- {green}[RANDOM]")
    print(f"      {red}[19] {red}REMOVE DUPLICATES         {yellow}- {green}[ACCURATE]")
    print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
    print(f"                      {red}ACCOUNT CHECKER ✅                                 ")
    print()
    print(f"      {white}[20] {green}ACCOUNT CHECKER         {yellow}- {red}[ACCURATE]")
    print(f"      {white}[21] {green}VALIDATE TOKENS         {yellow}- {red}[ACCURATE]")
    print(f"      {white}[22] {green}ACCOUNT REPORT          {yellow}- {red}[DETAILED]")
    while True:
        print(f"    {blue}───────────────────────────────────────────────────────────────\033[0m")
        choice = input(f"     {green}CHOICE (0 to exit): ").strip()         

        if choice == '0':
            print("    {yellow}Exiting... Goodbye!")
            break
        elif choice == '1':
            extraction()
        elif choice == '2':
        ...
        # Small pause to let user see results before returning to menu
        if choice != '0':
            input("    {yellow}Press Enter to return to main menu...")
            clear_screen()
            jovan()
            # Re-display the menu
